...
export type PixRequestScope = PixCommandScope & {
  container: RequestContainer
  organizationIds: string[]
  organizationScope: ResolvedOrganizationScope | null
}

class PixScopeError extends Error {
  readonly response: Response

  constructor(response: Response) {
    super('[internal] soanas_payments_br request scope rejected')
    this.response = response
  }
}

/** Resolves the tenant/organization scope of a Pix request; never trusts the body. */
export async function resolvePixRequestScope(req: Request): Promise<PixRequestScope> {
  const { translate } = await resolveTranslations()
  const container = await createRequestContainer()
  const auth = await getAuthFromRequest(req)
  if (!auth?.tenantId || !auth.sub) {
    throw new PixScopeError(
      NextResponse.json({ error: translate('soanas_payments_br.errors.unauthorized', 'Unauthorized') }, { status: 401 }),
    )
  }
  ...
  if (!organizationId) {
    throw new PixScopeError(
      NextResponse.json(
        { error: translate('soanas_payments_br.errors.org_required', 'Organization required') },
        { status: 400 },
      ),
    )
  }
...
export function pixScopeErrorResponse(err: unknown): Response | null {
  return err instanceof PixScopeError ? err.response : null
}

export type PixCommandRouteOptions<TSchema extends z.ZodTypeAny, TResult> = {
  schema: TSchema
  commandId: string
  /** Scope always comes from the session, never from the body. */
  buildInput: (body: Record<string, unknown>, scope: PixCommandScope) => Record<string, unknown>
  buildResponse: (result: TResult | undefined) => unknown
  status?: number
  failure: { key: string; fallback: string }
}

export async function handlePixCommandRoute<TSchema extends z.ZodTypeAny, TResult>(
  req: Request,
  options: PixCommandRouteOptions<TSchema, TResult>,
): Promise<Response> {
  const { translate } = await resolveTranslations()
  try {
    ...
    console.error('[soanas_payments_br] command route failure', options.commandId, debugMessage)
    return NextResponse.json({ error: translate(options.failure.key, options.failure.fallback) }, { status: 500 })
  }
}

/** Strips client-supplied scope keys so only server-derived values survive. */
export function withServerScope(
  body: Record<string, unknown>,
  scope: PixCommandScope,
  extra: Record<string, unknown> = {},
): Record<string, unknown> {
  const { tenantId: _tenantId, organizationId: _organizationId, ...rest } = body
  return {
    ...rest,
