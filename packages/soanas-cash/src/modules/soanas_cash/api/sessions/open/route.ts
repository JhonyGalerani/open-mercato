...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { result } = await commandBus.execute('soanas_cash.sessions.open', {
      input: parsed,
      ctx: {
        container,
        auth,
    ...
    return NextResponse.json({ id: result?.sessionId ?? null }, { status: 201 })
  } catch (err) {
    ...
    return NextResponse.json({ error: translate('soanas_cash.errors.open_failed', 'Failed to open cash session') }, { status: 500 })
  }
}
